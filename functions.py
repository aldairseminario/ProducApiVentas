...
def CargarFiltros(d):
    d = tablatemp(d)
    consulta = f"""SELECT DISTINCT Categoria, ZONA, TIPO FROM {d} WHERE `Tipodoc.` != "Nota de credito Boleta";"""
...
def GenerarFiltro(data):
    ...
    filtros = f"""WHERE `Tipodoc.` != "Nota de credito Boleta" AND FECHAREAL BETWEEN '{f1}' AND '{f2}'"""
    if z != []:
